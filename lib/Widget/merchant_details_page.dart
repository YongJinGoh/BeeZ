  ...
  Future<List<MerchantDetails>> getdatafromserver() async {
    ...
    if (!_isLoading) {
      ...
      if (responseData.code == 200) {
        ...
        if (dataFood['restorant'] == null) {
          coverImage = dataFood['cover'];

          openTime = trim(true, dataFood['working_hour']);
          closeTime = trim(false, dataFood['working_hour']);
          desc = dataFood['description'];

          if (dataFood['facebook'].toString().isNotEmpty &&
              dataFood['facebook'] != null) {
            socialList['facebook'] = dataFood['facebook'];
          }
          if (dataFood['instagram'] != '' && dataFood['instagram'] != null) {
            socialList['instagram'] = dataFood['instagram'];
          }
          if (dataFood['website'] != '' && dataFood['website'] != null) {
            socialList['website'] = dataFood['website'];
            website = dataFood['website'];
          }

          dataFood['categories'].forEach((mer) {
            itemLists = [];
          ...
          dataFood['time_list'].forEach((work) {
            openingHourLists.add(WorkingHour.fromJson(work));
          });

          dataFood['gallerys'].forEach((galItem) {
            galleryList.add(galItem);
          });
          address = dataFood['address'];
        } else {
